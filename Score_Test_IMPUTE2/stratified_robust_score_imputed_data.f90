...
!########################################################################################
!
!  File        : stratified_robust_score_imputed_data.f90
!
...
!########################################################################################
      ...
      MODULE METHODS
       ...
       PARAMETER( MAX_ALLELE_LENGTH = 100000 )
       ...
       CHARACTER( LEN = 32 ), ALLOCATABLE, DIMENSION( : ), SAVE :: SNP_NAMES, DATAG_NAMES
       CHARACTER( LEN = 32 ), ALLOCATABLE, DIMENSION( : ), SAVE :: ALLELE_FREQ
       CHARACTER( LEN = MAX_ALLELE_LENGTH + 1 ), ALLOCATABLE, DIMENSION( : ), SAVE :: ALLELE1, ALLELE2
       CHARACTER( LEN = 32 ), ALLOCATABLE, DIMENSION( : ), SAVE :: MAF, QUALITY, RSQ
       LOGICAL :: OVARIAN
...
!########################################################################################
      ...
      ! IT NOTES THE ORDER OF THE INDIVIDULS IN THE PHENO DATA, SO THAT THE GENO DATA CAN BE REORDERED
       ...
       LOGICAL :: FOUND
       LOGICAL :: STOP_ERROR
       !INTEGER, ALLOCATABLE, DIMENSION( : ) :: FOUND_STRAT
       ...
       OPEN( 22, FILE = TRIM( ADJUSTL( FILE_1 ) ), STATUS = 'OLD', ACTION = 'READ', IOSTAT = IO_STATUS )
       ...
       OPEN( 77, FILE = TRIM( ADJUSTL( FILE_2 ) ), STATUS = 'OLD', ACTION = 'READ', IOSTAT = IO_STATUS )
...
!########################################################################################
      ...
      SUBROUTINE SNP_INPUT( FILE_1, FILE_2, ERROR_STAT )
      ! THIS SUBROUTINE READS IN THE GENO DATA FROM THE SNP_INFO FILE AND THE DOSE_FILE
      ! IT REORDERS THE INDIVIDUALS IN THE DOSE FILE TO BE THE SAME AS THAT IN THE PHENO FILE

       IMPLICIT NONE
       INTEGER :: I, J, K, L, M
       INTEGER :: IO_STATUS, ALL_STATUS, ERROR_STAT
       INTEGER, ALLOCATABLE, DIMENSION( : ) :: ORDER_2
       REAL( KIND = KIND_REAL ) :: TEMP_R( 4 )
       CHARACTER( LEN = 32 ) :: WORD( 7 ), WORD_1
       CHARACTER( LEN = 256 ) :: FILE_1, FILE_2
       REAL( KIND = KIND_REAL ), ALLOCATABLE, DIMENSION( : ) :: TEMP_RA
       LOGICAL :: IS_NEW_FAM, IS_NEW_STR
 ...
 101   FORMAT( 'WARNING: FOR SNP "', A, '", ALLELE ', I1, ' IS TOO LONG' )
       ...
       !READ IN THE SNP NAMES
       OPEN( 88, FILE = TRIM( ADJUSTL( FILE_1 ) ), STATUS = 'OLD', &
            ACTION = 'READ', IOSTAT = IO_STATUS )
       IF( IO_STATUS .NE. 0 ) THEN
        CALL FILE_OPENING_ERROR( TRIM( FILE_1 ), 'SNP_INPUT', IO_STATUS )
        ERROR_STAT = 1
        RETURN
       ENDIF

       READ( 88, *, IOSTAT = IO_STATUS ) WORD
       IF( IO_STATUS .NE. 0 )THEN
        CALL FILE_READING_ERROR( TRIM( FILE_1 ), 'SNP_INPUT', -1, IO_STATUS )
        ERROR_STAT = 2
        RETURN
       ENDIF

       ! THE FIRST SWEEP THROUGH THE FILE COUNTS THE NUMBER OF SNPS ( N_SNP )
       N_SNP = 0
       IO_STATUS = 0
       DO
        READ( 88, *, IOSTAT = IO_STATUS ) WORD( 1 )
        IF( IO_STATUS .NE. 0 ) EXIT
        N_SNP = N_SNP + 1
       ENDDO

       IF( N_SNP .EQ. 0 ) THEN
        CALL FILE_READING_ERROR( TRIM( FILE_1 ), 'SNP_INPUT', -2, IO_STATUS )
        ERROR_STAT = 3
        RETURN
       ENDIF

       ! WRITE THE NUMBER OF SNPs TO THE SCREEN
       WRITE( *, * )'   NUMBER OF SNPs: ', N_SNP

       ! ALLOCATE SOME ARRAYS TO THE CORRECT SIZE
       IF( ALLOCATED( SNP_NAMES ) )THEN
        DEALLOCATE( SNP_NAMES, STAT = ALL_STATUS )
        IF( ALL_STATUS .NE. 0 ) CALL DEALLOCATION_ERROR( 'SNP_NAMES', 'SNP_INPUT', ALL_STATUS )
       ENDIF
       ALLOCATE( SNP_NAMES( N_SNP ), STAT = ALL_STATUS )
       IF( ALL_STATUS .NE. 0 ) CALL ALLOCATION_ERROR( 'SNP_NAMES', 'SNP_INPUT', ALL_STATUS )

       IF( ALLOCATED( TEMP_RA ) )THEN
        DEALLOCATE( TEMP_RA, STAT = ALL_STATUS )
        IF( ALL_STATUS .NE. 0 ) CALL DEALLOCATION_ERROR( 'TEMP_RA', 'SNP_INPUT', ALL_STATUS )
       ENDIF
       ALLOCATE( TEMP_RA( N_SNP ), STAT = ALL_STATUS )
       IF( ALL_STATUS .NE. 0 ) CALL ALLOCATION_ERROR( 'TEMP_RA', 'SNP_INPUT', ALL_STATUS )

       IF( ALLOCATED( GENO ) )THEN
        DEALLOCATE( GENO, STAT = ALL_STATUS )
        IF( ALL_STATUS .NE. 0 ) CALL DEALLOCATION_ERROR( 'GENO', 'SNP_INPUT', ALL_STATUS )
       ENDIF
       ALLOCATE( GENO( N_PEO, N_SNP ), STAT = ALL_STATUS )
       IF( ALL_STATUS .NE. 0 ) CALL ALLOCATION_ERROR( 'GENO', 'SNP_INPUT', ALL_STATUS )

       IF( ALLOCATED( ORDER_2 ) )THEN
        DEALLOCATE( ORDER_2, STAT = ALL_STATUS )
        IF( ALL_STATUS .NE. 0 ) CALL DEALLOCATION_ERROR( 'ORDER_2', 'SNP_INPUT', ALL_STATUS )
       ENDIF
       ALLOCATE( ORDER_2( N_PEO ), STAT = ALL_STATUS )
       IF( ALL_STATUS .NE. 0 ) CALL ALLOCATION_ERROR( 'ORDER_2', 'SNP_INPUT', ALL_STATUS )

       IF( ALLOCATED( ALLELE1 ) )THEN
        DEALLOCATE( ALLELE1, STAT = ALL_STATUS )
        IF( ALL_STATUS .NE. 0 ) CALL DEALLOCATION_ERROR( 'ALLELE1', 'SNP_INPUT', ALL_STATUS )
       ENDIF
       ALLOCATE( ALLELE1( N_SNP ), STAT = ALL_STATUS )
       IF( ALL_STATUS .NE. 0 ) CALL ALLOCATION_ERROR( 'ALLELE1', 'SNP_INPUT', ALL_STATUS )

       IF( ALLOCATED( ALLELE2 ) )THEN
        DEALLOCATE( ALLELE2, STAT = ALL_STATUS )
        IF( ALL_STATUS .NE. 0 ) CALL DEALLOCATION_ERROR( 'ALLELE2', 'SNP_INPUT', ALL_STATUS )
       ENDIF
       ALLOCATE( ALLELE2( N_SNP ), STAT = ALL_STATUS )
       IF( ALL_STATUS .NE. 0 ) CALL ALLOCATION_ERROR( 'ALLELE2', 'SNP_INPUT', ALL_STATUS )

       IF( ALLOCATED( ALLELE_FREQ ) )THEN
        DEALLOCATE( ALLELE_FREQ, STAT = ALL_STATUS )
        IF( ALL_STATUS .NE. 0 ) CALL DEALLOCATION_ERROR( 'ALLELE_FREQ', 'SNP_INPUT', ALL_STATUS )
       ENDIF
       ALLOCATE( ALLELE_FREQ( N_SNP ), STAT = ALL_STATUS )
       IF( ALL_STATUS .NE. 0 ) CALL ALLOCATION_ERROR( 'ALLELE_FREQ', 'SNP_INPUT', ALL_STATUS )

       IF( ALLOCATED( MAF ) )THEN
        DEALLOCATE( MAF, STAT = ALL_STATUS )
        IF( ALL_STATUS .NE. 0 ) CALL DEALLOCATION_ERROR( 'MAF', 'SNP_INPUT', ALL_STATUS )
       ENDIF
       ALLOCATE( MAF( N_SNP ), STAT = ALL_STATUS )
       IF( ALL_STATUS .NE. 0 ) CALL ALLOCATION_ERROR( 'MAF', 'SNP_INPUT', ALL_STATUS )

       IF( ALLOCATED( QUALITY ) )THEN
        DEALLOCATE( QUALITY, STAT = ALL_STATUS )
        IF( ALL_STATUS .NE. 0 ) CALL DEALLOCATION_ERROR( 'QUALITY', 'SNP_INPUT', ALL_STATUS )
       ENDIF
       ALLOCATE( QUALITY( N_SNP ), STAT = ALL_STATUS )
       IF( ALL_STATUS .NE. 0 ) CALL ALLOCATION_ERROR( 'QUALITY', 'SNP_INPUT', ALL_STATUS )

       IF( ALLOCATED( RSQ ) )THEN
        DEALLOCATE( RSQ, STAT = ALL_STATUS )
        IF( ALL_STATUS .NE. 0 ) CALL DEALLOCATION_ERROR( 'RSQ', 'SNP_INPUT', ALL_STATUS )
       ENDIF
       ALLOCATE( RSQ( N_SNP ), STAT = ALL_STATUS )
       IF( ALL_STATUS .NE. 0 ) CALL ALLOCATION_ERROR( 'RSQ', 'SNP_INPUT', ALL_STATUS )



       ! REWIND THE FILE FOR THE SECOND SWEEP
       REWIND( 88 )
       WRITE( *, * )'   READING IN THE SNP INFO.'

       READ( 88, *, IOSTAT = IO_STATUS ) WORD
       IF( IO_STATUS .NE. 0 ) THEN
        CALL FILE_READING_ERROR( TRIM( FILE_1 ), 'SNP_INPUT', -3, IO_STATUS )
        IF( ALLOCATED( TEMP_RA ) )THEN
         DEALLOCATE( TEMP_RA, STAT = ALL_STATUS )
         IF( ALL_STATUS .NE. 0 ) CALL DEALLOCATION_ERROR( 'TEMP_RA', 'SNP_INPUT', ALL_STATUS )
        ENDIF
       ...
       DO I = 1, N_SNP
        READ( 88, *, IOSTAT = IO_STATUS ) SNP_NAMES( I ), ALLELE1( I ), ALLELE2( I ), ALLELE_FREQ( I ), &
                                          MAF( I ), QUALITY( I ), RSQ( I )

        ! CHECK THAT THERE THE ENTIRE ALLELE HAS BEEN READ IN
        IF( LEN_TRIM( ADJUSTL( ALLELE1( I ) ) ) .GT. MAX_ALLELE_LENGTH ) THEN
         WRITE( *, 101 ) TRIM( ADJUSTL( SNP_NAMES( I ) ) ), 1 
         ALLELE1( I ) = 'WARNING'
        ENDIF
        IF( LEN_TRIM( ADJUSTL( ALLELE2( I ) ) ) .GT. MAX_ALLELE_LENGTH ) THEN
         WRITE( *, 101 ) TRIM( ADJUSTL( SNP_NAMES( I ) ) ), 2 
         ALLELE2( I ) = 'WARNING'
        ENDIF

        IF( IO_STATUS .NE. 0 ) THEN
         CALL FILE_READING_ERROR( TRIM( FILE_1 ), 'SNP_INPUT', I, IO_STATUS )
         IF( ALLOCATED( TEMP_RA ) )THEN
          DEALLOCATE( TEMP_RA, STAT = ALL_STATUS )
          IF( ALL_STATUS .NE. 0 ) CALL DEALLOCATION_ERROR( 'TEMP_RA', 'SNP_INPUT', ALL_STATUS )
         ENDIF
       ...
       !READ IN GENOTYPIC DATA FROM THE DOSE FILE, AND PUT IT INTO THE CORRECT ORDER
       OPEN( 99, FILE = TRIM( ADJUSTL( FILE_2 ) ), STATUS = 'OLD', &
             ACTION = 'READ', IOSTAT = IO_STATUS )
       IF( IO_STATUS .NE. 0 )THEN
        CALL FILE_OPENING_ERROR( TRIM( FILE_2 ), 'SNP_INPUT', IO_STATUS )
        IF( ALLOCATED( TEMP_RA ) )THEN
         DEALLOCATE( TEMP_RA, STAT = ALL_STATUS )
         IF( ALL_STATUS .NE. 0 ) CALL DEALLOCATION_ERROR( 'TEMP_RA', 'SNP_INPUT', ALL_STATUS )
        ENDIF
       ...
       WRITE( *, * )'   READING IN THE DOSE INFO.'
       K = 0
       L = 0
       ORDER_2 = 0
       DO 
        WORD_1 = ''
        L = L + 1
        READ(99, *, IOSTAT = IO_STATUS) WORD(1), WORD(3), TEMP_RA

        IF(IO_STATUS .NE. 0)THEN
         CALL FILE_READING_ERROR(TRIM(FILE_2), 'SNP_INPUT', L, IO_STATUS)
         IF(ALLOCATED(TEMP_RA))THEN
          DEALLOCATE(TEMP_RA, STAT = ALL_STATUS)
          IF(ALL_STATUS .NE. 0) CALL DEALLOCATION_ERROR('TEMP_RA', 'SNP_INPUT', ALL_STATUS)
         ENDIF
        ...
        DO I = 1, N_PEO
         IF(TRIM(ADJUSTL(WORD(1))) .EQ.  DATAG_NAMES(I)) THEN
          GENO(ORDER(I), :) = TEMP_RA
          ORDER_2(ORDER(I)) = I
          K = K + 1
          EXIT
         ENDIF
       ...
       ! CHECK THAT EACH INDIVIDUAL HAS BEEN FOUND, IF NOT RETRUN, WITH AN ERROR STAT \neq 0    
       DO I = 1, N_PEO
        IF(ORDER_2(I) .EQ. 0) THEN
         WRITE(*, *)'PERSON ', I,' IS NOT IN THE GENO FILE', TRIM(ADJUSTL(FILE_2))
         IF(ALLOCATED(TEMP_RA))THEN
          DEALLOCATE(TEMP_RA, STAT = ALL_STATUS)
          IF(ALL_STATUS .NE. 0) CALL DEALLOCATION_ERROR('TEMP_RA', 'SNP_INPUT', ALL_STATUS)
         ENDIF
...
!########################################################################################
      ...
      ! THIS SUBROUTINE DEALLOCATES ARRAYS
       ...
       IF( ALLOCATED( SNP_NAMES ) ) THEN
        DEALLOCATE( SNP_NAMES, STAT = ALL_STATUS )
        IF( ALL_STATUS .NE. 0 ) CALL DEALLOCATION_ERROR( 'SNP_NAMES', 'CLEAN_METHODS', ALL_STATUS )
       ENDIF
       ...
       IF( ALLOCATED( ALLELE1 ) ) THEN
        DEALLOCATE( ALLELE1, STAT = ALL_STATUS )
        IF( ALL_STATUS .NE. 0 ) CALL DEALLOCATION_ERROR( 'ALLELE1', 'CLEAN_METHODS', ALL_STATUS )
       ENDIF

       IF( ALLOCATED( ALLELE2 ) ) THEN
        DEALLOCATE( ALLELE2, STAT = ALL_STATUS )
        IF( ALL_STATUS .NE. 0 ) CALL DEALLOCATION_ERROR( 'ALLELE2', 'CLEAN_METHODS', ALL_STATUS )
       ENDIF

       IF( ALLOCATED( ALLELE_FREQ ) ) THEN
        DEALLOCATE( ALLELE_FREQ, STAT = ALL_STATUS )
        IF( ALL_STATUS .NE. 0 ) CALL DEALLOCATION_ERROR( 'ALLELE_FREQ', 'CLEAN_METHODS', ALL_STATUS )
       ENDIF

       IF( ALLOCATED( MAF ) ) THEN
        DEALLOCATE( MAF, STAT = ALL_STATUS )
        IF( ALL_STATUS .NE. 0 ) CALL DEALLOCATION_ERROR( 'MAF', 'CLEAN_METHODS', ALL_STATUS )
       ENDIF

       IF( ALLOCATED( QUALITY ) ) THEN
        DEALLOCATE( QUALITY, STAT = ALL_STATUS )
        IF( ALL_STATUS .NE. 0 ) CALL DEALLOCATION_ERROR( 'QUALITY', 'CLEAN_METHODS', ALL_STATUS )
       ENDIF

       IF( ALLOCATED( RSQ ) ) THEN
        DEALLOCATE( RSQ, STAT = ALL_STATUS )
        IF( ALL_STATUS .NE. 0 ) CALL DEALLOCATION_ERROR( 'RSQ', 'CLEAN_METHODS', ALL_STATUS )
       ENDIF
...
!########################################################################################
      ...
      INTEGER :: ALL_STATUS, IO_STATUS, ERROR_STAT
      REAL( KIND = KIND_REAL ) :: VAR_G, DENOM( 3 ), NUMR, TEMP
      REAL( KIND = KIND_REAL ), ALLOCATABLE, DIMENSION( : ) :: G
      REAL( KIND = KIND_REAL ), ALLOCATABLE, DIMENSION( :, : ) :: C, NUM
      CHARACTER( LEN = 32 ) :: WORDS( 7 )
      CHARACTER( LEN = 256 ) :: PHENO_FILE, SNP_INFO_FILE, GENO_FILE, INPUT_FILE
      CHARACTER( LEN = 256 ) :: KINSHIP_FILE, RESULTS_FILE, OUT_FILE
      LOGICAL :: FILE_EXIST

 100  FORMAT( 14( A, :',' ) )

      NAMELIST / filenames / PHENO_FILE, KINSHIP_FILE, SNP_INFO_FILE, &
                             GENO_FILE, RESULTS_FILE, OVARIAN

      INPUT_FILE = "file_names.nml"
      ...
      WRITE( *, * ) 'SNP_INFO_FILE: "'//TRIM( ADJUSTL( SNP_INFO_FILE ) )//'"'
      ...
      INQUIRE( FILE = TRIM( ADJUSTL( SNP_INFO_FILE ) ), EXIST = FILE_EXIST )
      IF( .NOT. FILE_EXIST ) THEN
       WRITE( *, * ) 'ERROR: THE FILE "'//TRIM( ADJUSTL( SNP_INFO_FILE ) )//'" DOES NOT EXIST'
       WRITE( *, * ) 'STOPPING PREMATURELY.'
       CALL CLEAN_METHODS( )
       CALL EXIT( 13 )
      ENDIF
      ...
      DO I = 1, N_STRAT
       ...
       NUM( I, 1 ) = DOT( PHENO( STRAT_START_POSN( I ):STRAT_START_POSN( I ) + N_PEO_STRAT( I ) - 1 ), &
                          N_PEO_STRAT( I ) )
      ...
      WRITE( *, * )'READING IN THE GENO DATA.'

      ERROR_STAT = 0
      CALL SNP_INPUT( SNP_INFO_FILE, GENO_FILE, ERROR_STAT )

      IF( ERROR_STAT .NE. 0 ) THEN
       WRITE( *, * )'ERROR READING IN GENO DATA.'
       WRITE( *, * )'STOPPING PREMATURELY.'
       CALL CLEAN_METHODS( )
       CALL EXIT( 10 )
      ENDIF
      ...
      DO J = 1, N_SNP
       ...
       ! TAKE THE SLICE OF THE GENO MATRIX CORRESPONDING TO THIS SNP
       G = GENO( :, J )
       ...
       WRITE( 777, 100 ) TRIM( ADJUSTL( SNP_NAMES( J ) ) ), &
                         TRIM( ADJUSTL( ALLELE1( J ) ) ), TRIM( ADJUSTL( ALLELE2( J ) ) ), &
                         TRIM( ADJUSTL( ALLELE_FREQ( J ) ) ), TRIM( ADJUSTL( MAF( J ) ) ),  &
                         TRIM( ADJUSTL( QUALITY( J ) ) ), TRIM( ADJUSTL( RSQ( J ) ) ), &
